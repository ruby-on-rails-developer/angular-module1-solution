## Module 1 Coding solution

### Introduction
The front-end application presents the user with a textbox where they can list comma-separated items they usually eat for lunch. Once that's entered, the user has to click the "Check If Too Much" button.
...
### Note: bonus feature
You are not required to handle a case where there is no item between some commas.  However, you can implement checking for an 'empty' item has been implemented. For example, the case `item 1, item2,,item3` or this case `item 1, item2, ,item3` as 3 items in the list. An empty item, i.e., `, ,` is not counted as an item towards to the count.
